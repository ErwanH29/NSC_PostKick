...
class HyperbolicPlotter(object):
    ...
    def extract_folder(self, SMBH_mass, vkick):
        ...
        Args:
            ...
            vkick (str):  Velocity kick of the ejected BH
        Returns:
            merge_txt (list):  List of text files with merger data
            merge_hdf5 (list):  List of hdf5 files with merger data
        """
    ...
    def make_planar(self, sma, ecc, Omega, omega, inc):
        """
        Function to make a planar orbit
        Args:
            sma (units.length): Semi-major axis of the orbit
            ecc (float): Eccentricity of the orbit
            Omega (units.rad): Longitude of the ascending node
            omega (units.rad): Argument of periapsis
            inc (units.rad): Inclination of the orbit
        Returns:
    ...
    def sma_ecc_hyperbolic_colls(self):
        """Plot the semi-major axis and eccentricity of hyperbolic collisions"""
        vkicks = [300, 600]
        SMBH_masses = ["1e5", "4e5"]
        
        fig_path = "plot/figures/temp/hyperbolic_orbit_{}.png"
        Ncoll = 0
        for kick in vkicks:
                ...
                for config_txt, config_hdf5 in zip(configs_txt, configs_hdf5):
                    ...
                    for txt, hdf5 in zip(text_file, hdf5_file):
                        ...
                        for coll_ in [coll_a, coll_b]:
                            ...
                            x, y, z = self.make_planar(sma, ecc, loa, aop, inc)
                            coll_x_arr.append(x)
                            coll_y_arr.append(y)
                            coll_z_arr.append(z)
                        ...
                        x_c, y_c, z_c = self.make_planar(sma, ecc, loa, aop, inc)
                        ...
                        plt.savefig(fig_path.format(str(Ncoll)+"_3D"), bbox_inches="tight", dpi=300)
                        plt.close()
                        ...
                        plt.savefig(fig_path.format(Ncoll), bbox_inches="tight", dpi=300)
                        plt.close()
    
    def plot_evolution(self):
        """Plot the evolution of hyperbolic orbits"""
